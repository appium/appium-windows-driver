...
const getWADExecutablePath = _.memoize(async function getWADInstallPath () {
  ...
  const pathCandidates = POSSIBLE_WAD_INSTALL_ROOTS
    ...
    .filter(Boolean)
    // construct full path
    .map((root) => path.resolve(root, 'Windows Application Driver', WAD_EXECUTABLE_NAME));
  for (const result of pathCandidates) {
    if (await fs.exists(result)) {
      return result;
    }
  }
  throw new WADNotFoundError(`${WAD_EXECUTABLE_NAME} has not been found in any of these ` +
...
export {
  ...
  getWADExecutablePath, WAD_GUID, isAdmin,
};
