...
export async function getWindowSize () {
  const size = await /** @type {WinAppDriver} */ (this.winAppDriver).sendCommand(
    '/window/size', 'GET'
  );
...
export async function getWindowRect () {
  ...
  let [x, y] = [0, 0];
  try {
    const handle = await /** @type {WinAppDriver} */ (this.winAppDriver).sendCommand(
      '/window_handle', 'GET'
    );
    ({x, y} = /** @type {import('@appium/types').Position} */ (
      await /** @type {WinAppDriver} */ (this.winAppDriver).sendCommand(
...
export async function getScreenshot () {
  ...
  const originalPayload = await /** @type {WinAppDriver} */ (this.winAppDriver).sendCommand(
    '/screenshot', 'GET'
  );
