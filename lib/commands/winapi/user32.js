...
// Mostly ported from https://chromium.googlesource.com/chromium/src/+/refs/heads/main/remoting/host/input_injector_win.cc
...
const INPUT_STRUCT_SIZE = arch === 'x64' ? 40 : 28;
...
async function sendInputs (inputs, many = true) {
  ...
  const uSent = await new B((resolve, reject) => {
    ...
    user32.SendInput.async(inputsCount, inputs, INPUT_STRUCT_SIZE, (error, result) => error ? reject(error) : resolve(result));
  });
...
function toKeyInput({virtualKeyCode, action}) {
  ...
  switch (action) {
    ...
    case KEY_ACTION.DOWN:
      break;
    default:
      throw new Error(`Key action '${action}' is unknown. Only ${_.values(KEY_ACTION)} are supported`);
  }
...
export async function toMouseButtonInput({button, action}) {
  ...
  switch (action) {
    ...
    case MOUSE_BUTTON_ACTION.DOWN:
      break;
    default:
      throw new Error(`Mouse button action '${action}' is unknown. Only ${_.values(MOUSE_BUTTON_ACTION)} are supported`);
  }
  ...
  switch (button) {
    ...
    case MOUSE_BUTTON.FORWARD:
      ...
      break;
    default:
      throw new Error(`Mouse button '${button}' is unknown. Only ${_.values(MOUSE_BUTTON)} are supported`);
  }
