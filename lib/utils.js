...
export async function runElevated(cmd, args = [], opts = {}) {
  ...
  const psArgList = buildPSArgumentList(args);
...
/**
 * Escapes a string to be used as a PowerShell single-quoted string
 *
 * @param {string} str
 * @returns {string}
 */
...
/**
 * Build the PowerShell -ArgumentList value from an array of args.
 * Produces a comma-separated list of single-quoted entries:
 * 'arg1','arg2','arg with spaces'
 *
 * @param {string[]} args
 * @returns {string}
 */
function buildPSArgumentList(args) {
  if (!Array.isArray(args) || args.length === 0) {
    return "''"; // Pass empty string argument list
  }
  return args.map(escapePSSingleQuoted).join(',');
}
